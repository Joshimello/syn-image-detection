...
CONFIG_PATH="${1:-configs/open_truefake_social.yaml}"

if [[ "${CONFIG_PATH}" == "-h" || "${CONFIG_PATH}" == "--help" ]]; then
  echo "Usage: ./scripts/run_open_truefake_social.sh [config_path]"
  echo "Default config: configs/open_truefake_social.yaml"
  exit 0
fi
...
print(raw.get("train", {}).get("run_name", "open_clip_l14_truefake_social"))
PY
)"
...
echo "Running open pipeline with config: ${CONFIG_PATH}"
