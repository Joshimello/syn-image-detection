...
uv run me26sid-ensemble \
  --config configs/constrained_smallimg_trainonly.yaml \
  --run-name "recal_smallimg_trainonly_exact_${RUN_STAMP}" \
  --source-run "${BEST_RUN}"

uv run me26sid-ensemble \
  --config configs/constrained_unfreeze_last2.yaml \
  --run-name "recal_unfreeze_last2_exact_${RUN_STAMP}" \
  --source-run "${UNFREEZE_LAST2_RUN}"

uv run me26sid-ensemble \
  --config configs/constrained_smallimg_moredata.yaml \
...
if [[ "${WITH_TRAINING}" == "1" ]]; then
  ...
  ./scripts/run_constrained.sh configs/constrained_unfreeze_last2_smallimg_moredata.yaml
  ./scripts/run_constrained.sh configs/constrained_unfreeze_last2_backbone5e6.yaml
else
